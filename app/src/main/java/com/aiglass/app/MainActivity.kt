...
class MainActivity : ComponentActivity(), TextToSpeech.OnInitListener {
    ...
    private fun setupCallbacks() {
        ...
        lifecycleScope.launch {
            webSocketManager.aiSpeechText.collect { response ->
                if (response != null) {
                    speakAiReply(response)
                }
            }
        }
        lifecycleScope.launch {
        ...
        webSocketManager.onAiMessage = { msg -> Log.d(TAG, "AI reply received: $msg") }
    ...
    private fun speakAiReply(text: String) {
        val cleanText = text.trim()
        if (!ttsReady || cleanText.isEmpty()) return
        textToSpeech?.stop()
        textToSpeech?.speak(cleanText, TextToSpeech.QUEUE_FLUSH, null, "ai_reply")
    }
    ...
    private fun startAudioStream() {
        ...
        audioPlayback.playStreamFromUrl(
            ...
            shouldReconnect = { isConnected && connectionState.isFullyConnected },
