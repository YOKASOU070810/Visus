...
# --- mediapipe 0.10.x 兼容层：旧版 solutions/framework 已移除 ---
# 手部连接关系（21个关键点之间的连线）
_HAND_CONNECTIONS = [
    (0,1),(1,2),(2,3),(3,4),         # 拇指
    (0,5),(5,6),(6,7),(7,8),         # 食指
...
# 假兼容层的 NormalizedLandmarkList — 让 draw_landmarks 接受普通 list
class _FakeLandmarkList:
    def __init__(self, lms):
        self.landmark = lms

def _fake_draw_landmarks(img, landmark_list, connections, landmark_drawing_spec, connection_drawing_spec):
    h, w = img.shape[:2]
    color_lm = landmark_drawing_spec.color if hasattr(landmark_drawing_spec, 'color') else (0,255,255)
    r = landmark_drawing_spec.circle_radius if hasattr(landmark_drawing_spec, 'circle_radius') else 2
    color_conn = connection_drawing_spec.color if hasattr(connection_drawing_spec, 'color') else (0,255,255)
    t = connection_drawing_spec.thickness if hasattr(connection_drawing_spec, 'thickness') else 2
    for lm in landmark_list.landmark:
        x, y = int(lm.x * w), int(lm.y * h)
        cv2.circle(img, (x, y), r, color_lm, -1)
    for i, j in connections:
        x1, y1 = int(landmark_list.landmark[i].x * w), int(landmark_list.landmark[i].y * h)
        x2, y2 = int(landmark_list.landmark[j].x * w), int(landmark_list.landmark[j].y * h)
        cv2.line(img, (x1, y1), (x2, y2), color_conn, t)

class _FakeDrawingSpec:
    def __init__(self, color=(0,255,255), thickness=2, circle_radius=2):
        self.color = color
        self.thickness = thickness
        self.circle_radius = circle_radius

from voice.audio_player import play_audio_threadsafe
...
def _to_list(hand_lms):
    """将 hand_lms 统一转为 _FakeLandmarkList"""
    if hasattr(hand_lms, "landmark"):
        return hand_lms
    return _FakeLandmarkList(hand_lms)

# —— 手骨架单色渲染 —— #
def draw_hands_mono(img_bgr, hand_lms, color=(0, 255, 255), r=2, t=2):
    landmark_spec   = _FakeDrawingSpec(color=color, thickness=-1, circle_radius=r)
    connection_spec = _FakeDrawingSpec(color=color, thickness=t,  circle_radius=r)
    _fake_draw_landmarks(
        img_bgr,
        landmark_list=_to_list(hand_lms),
