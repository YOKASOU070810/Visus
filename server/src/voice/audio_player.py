...
import queue
import time
from voice.audio_stream import broadcast_pcm16_realtime
from audio.audio_compressor import compressed_audio_cache, AudioCompressor

# Python 3.13+ compatibility: audioop removed per PEP 594
import sys
try:
    import audioop
...
def _get_recorder():
    ...
    if not _recorder_imported:
        try:
            from audio import sync_recorder as sr
            _sync_recorder = sr
...
# 兼容旧工程中的示例音频（保留）
AUDIO_BASE_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), "..", "..", "assets", "audio_prompts", "navigation")

# 新增：voice 目录与映射表
# 使用脚本所在目录的 voice 文件夹，避免工作目录问题
VOICE_DIR = os.getenv("VOICE_DIR", os.path.join(os.path.dirname(os.path.abspath(__file__)), "..", "..", "assets", "audio_prompts", "system"))
